...
function renderTomorrow() {
  const cards = [
    {
      icon: '🫀', label: 'Cath Lab Tomorrow', value: snapshot.cathLabTomorrow || 0,
      color: 'var(--red)', bg: 'var(--red-light)',
      badge: 'Tomorrow', badgeText: '#C2483D'
    },
    {
      icon: '🔭', label: 'Endoscopy Tomorrow', value: snapshot.endoTomorrow || 0,
      color: '#6B4FA0', bg: '#EFEAF7',
      badge: 'Tomorrow', badgeText: '#6B4FA0'
    },
  ];
  document.getElementById('tomorrowGrid').innerHTML = cards.map(c =>
    '<div class="proc-card">' +
    '<div class="proc-accent" style="background:' + c.color + '"></div>' +
    '<span class="proc-icon">' + c.icon + '</span>' +
    '<div class="proc-value">' + c.value + '</div>' +
